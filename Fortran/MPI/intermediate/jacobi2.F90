program jacobi2
	...
	do n=1,steps
		!从左向右平移数据
		if (myid == 0) then
			call MPI_SEND(a(1,mysize+1),totalsize,MPI_REAL,myid+1,&
				10,MPI_COMM_WORLD,ierr)
		else if (myid == 3) then
			call MPI_RECV(a(1,1),totalsize,MPI_REAL,myid-1,&
				10,MPI_COMM_WORLD,status,ierr)
		else
			call MPI_SENDRECV(a(1,mysize+1),totalsize,MPI_REAL,myid+1,&
				10,a(1,1),totalsize,MPI_REAL,myid-1,&
				10,MPI_COMM_WORLD,status,ierr)
		end if
		!从右向左平移数据
		if (myid == 0) then
			call MPI_RECV(a(1,mysize+2),totalsize,MPI_REAL,myid+1,&
				10,MPI_COMM_WORLD,status,ierr)
		else if (myid == 3) then
			call MPI_SEND(a(1,2),totalsize,MPI_REAL,myid-1,&
				10,MPI_COMM_WORLD,ierr)
		else
			call MPI_SENDRECV(a(1,2),totalsize,MPI_REAL,myid-1,&
				10,a(1,mysize+2),totalsize,MPI_REAL,myid+1,&
				10,MPI_COMM_WORLD,status,ierr)
		end if
