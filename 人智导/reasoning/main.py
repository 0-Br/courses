...
def update_params_for_one_item(K, n_word_types, word_indices, counts, gammas, phi_d, log_betas, alpha, max_iter_d,
    ...
    while i < max_iter_d and delta >= tol_d:
        ...
        while n < n_word_types:
            ...
            new_phi_dn = np.exp(log_betas[w] + psi_gammas)
            phi_d[n] = new_phi_dn / np.sum(new_phi_dn)

            n += 1
        gammas = alpha + np.sum(phi_d * counts.reshape((-1, 1)), axis=0)
        psi_gammas = psi(gammas)
...
def compute_bound_for_one_item(K, n_word_types, word_indices, count_vector, alpha, gammas, psi_gammas, phi_d,
    ...
    for n in range(n_word_types):
        w = word_indices[n]
        c = count_vector[n]
        E_word += np.sum(phi_d[n] * c * log_betas[w])

    bound = (gammaln(K * alpha) - K * gammaln(alpha) + (alpha - 1) * np.sum(psi(gammas) - psi(np.sum(gammas)))
            + np.sum(count_vector.reshape((-1, 1)) * phi_d * (psi(gammas) - psi(np.sum(gammas))))
            + E_word
            - gammaln(np.sum(gammas)) + np.sum(gammaln(gammas)) - np.sum((gammas - 1) * (psi(gammas) - psi(np.sum(gammas))))
            - np.sum(count_vector.reshape((-1, 1)) * phi_d * np.log(phi_d)))
