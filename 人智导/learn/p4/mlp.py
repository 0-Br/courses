...
def softmax(x:np.array) -> np.array:
    re0 = np.exp(x)
    return re0 / np.sum(re0, axis = 1).reshape(-1, 1)

def p_sigmoid(x:np.array) -> np.array:
    re0 = np.exp(x)
    return re0 / ((1 + re0) * (1 + re0))
...
def calc_loss_and_grad(x, y, w1, w2, eval_only=False):
    ...
    loss = np.sum(y * -np.log(y_hat)) / args.batch_size

    if eval_only:
        return loss, y_hat

    dw2 = np.matmul(h.T, softmax(o) - y)

    dw1 = np.matmul(x.T, np.matmul(softmax(o) - y, w2.T) * p_sigmoid(z))
