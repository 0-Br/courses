...
class TreeNode(object):
    ...
    def get_ucb(self, c):
        ...
        """
        return (-self.U / self.n_visits + c * np.sqrt(np.log10(self.parent.n_visits) / self.n_visits))
    ...
    def select(self, c):
        ...
        """
        ucb_max = -100000000.0
        action_next, next_node = None, None
        for action, child in self.children.items():
            if child.get_ucb(c) > ucb_max:
                action_next, next_node = action, child
                ucb_max = child.get_ucb(c)
        return (action_next, next_node)
