    ...
    def get_action(self, state: State):
        ...
        def minimax_search(s: State) -> Tuple:
            ...
            if end:
                ...
                if self.player == s.get_current_player():
                    value = -inf
                    for a in actions:
                        s_next = deepcopy(s).perform_action(a)
                        value_next, action_next = minimax_search(s_next)
                        if value_next > value:
                            value = value_next
                            action = a
                else:
        ...
        def alpha_beta_search(s: State, alpha, beta):
            ...
            if end:
                ...
                if self.player == s.get_current_player():
                    ...
                    for a in actions:
                        s_next = deepcopy(s).perform_action(a)
                        value_next, action_next = alpha_beta_search(s_next, alpha, beta)
                        if value_next > value:
                            value = value_next
                            action = a
                        if value_next >= beta:
            ...
            if end:
                ...
                if d == 0:
                    ...
                    if self.player == s.get_current_player():
                        ...
                        for a in actions:
                            s_next = deepcopy(s).perform_action(a)
                            value_next, action_next = cutting_off_alpha_beta_search(s_next, d, alpha, beta)
                            if value_next > value:
                                value = value_next
                                action = a
                            if value_next >= beta:
                        ...
                        for a in actions:
                            s_next = deepcopy(s).perform_action(a)
                            value_next, action_next = cutting_off_alpha_beta_search(s_next, d - 1, alpha, beta)
                            if value_next < value:
                                value = value_next
                                action = a
                            if value_next <= alpha:
