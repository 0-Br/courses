...
class Model:
    ...
    def init_weights(self):
        ...
        self.b_s1 = np.random.normal(loc=0, scale=np.sqrt(2 / self.seq_len), size=self.hidden)
        self.w_s2 = np.random.normal(loc=0, scale=np.sqrt(2 / self.hidden), size=(self.hidden, self.pred_len))
        self.b_s2 = np.random.normal(loc=0, scale=np.sqrt(2 / self.hidden), size=self.pred_len)

        self.w_t1 = np.random.normal(loc=0, scale=np.sqrt(2 / self.seq_len), size=(self.seq_len, self.hidden))
        self.b_t1 = np.random.normal(loc=0, scale=np.sqrt(2 / self.seq_len), size=self.hidden)
        self.w_t2 = np.random.normal(loc=0, scale=np.sqrt(2 / self.hidden), size=(self.hidden, self.pred_len))
        self.b_t2 = np.random.normal(loc=0, scale=np.sqrt(2 / self.hidden), size=self.pred_len)
    ...
    def forward_backward(self, x, y, forward_only=False):
        ...
        z_s1 = seasonal_init @ self.w_s1 + self.b_s1
        a_s1 = z_s1 * (z_s1 > 0)
        z_s2 = a_s1 @ self.w_s2 + self.b_s2
        seasonal_output = z_s2

        z_t1 = trend_init @ self.w_t1 + self.b_t1
        a_t1 = z_t1 * (z_t1 > 0)
        z_t2 = a_t1 @ self.w_t2 + self.b_t2
        trend_output = z_t2
        ...
        if not forward_only:
            ...
            delta_s2 = (2 / (x.shape[0] * x.shape[1])) * (x - np.transpose(y, (0, 2, 1)).reshape(-1, self.pred_len)) # (BN, S_p)
            self.b_s2_grad = np.sum(delta_s2, axis=0) # (S_p)
            self.w_s2_grad = a_s1.T @ delta_s2 # (H, BN) @ (BN, S_p) = (H, S_p)
            delta_s1 = (delta_s2 @ self.w_s2.T) * np.heaviside(z_s1, 0) # ((BN, S_p) @ (S_p, H)) * (BN, H) = (BN, H)
            self.b_s1_grad = np.sum(delta_s1, axis=0) # (H)
            self.w_s1_grad = seasonal_init.T @ delta_s1 # (S, BN) @ (BN, H) = (S, H)

            delta_t2 = (2 / (x.shape[0] * x.shape[1])) * (x - np.transpose(y, (0, 2, 1)).reshape(-1, self.pred_len))
            self.b_t2_grad = np.sum(delta_t2, axis=0)
            self.w_t2_grad = a_t1.T @ delta_t2
            delta_t1 = (delta_t2 @ self.w_t2.T) * np.heaviside(z_t1, 0)
            self.b_t1_grad = np.sum(delta_t1, axis=0)
            self.w_t1_grad = trend_init.T @ delta_t1
