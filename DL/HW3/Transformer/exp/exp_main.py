...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            valid_writer.add_scalar("Loss/valid", vali_loss, epoch)
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    x /= stdev
                    enc_out = self.model.enc_embedding(x, batch_x_mark)
                    layers = nn.Sequential(*list(self.model.encoder.attn_layers)[:-1])
                    input_attn = layers(enc_out)
                    attn_weights = self.model.encoder.attn_layers[-1].attention(input_attn[0], input_attn[0], input_attn[0])[1][0, 0, :, :].detach().cpu().numpy()
                    plt.imshow(attn_weights, vmin = np.min(attn_weights), vmax = np.max(attn_weights))
                    plt.xticks(np.arange(0, 96), fontsize=4)
                    plt.yticks(np.arange(0, 96), fontsize=4)
                    plt.colorbar()
                    plt.xlabel("Key")
                    plt.ylabel("Query")
