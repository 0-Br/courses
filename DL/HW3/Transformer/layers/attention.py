...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q_data, k_data, v_data, attn_mask=None):
        ...
        S = q_data.shape[1]
        T = k_data.shape[1]
        query, key, value = (self.Wq(q_data).reshape([N, S, self.n_head, self.head_dim]).transpose(1, 2),
                             self.Wk(k_data).reshape([N, T, self.n_head, self.head_dim]).transpose(1, 2),
                             self.Wv(v_data).reshape([N, T, self.n_head, self.head_dim]).transpose(1, 2))
        ############################################################################
...
def multi_head_attention(query, key, value, attn_mask=None, dropout=0.1):
    ...
    ############################################################################
    attention = torch.matmul(query, key.transpose(2, 3)) / np.sqrt(query.shape[3]) # -> (N, H, T, T)

    # sparse attention改进
    window_mask = torch.ones((S, T)).triu(-48).tril(48).to(attention.device) # 取k=48
    window_mask = window_mask.unsqueeze(0).unsqueeze(0)
    attention = attention * window_mask

    if attn_mask is not None:
        attention = attention.masked_fill(torch.logical_not(attn_mask), -torch.inf)
    attention = nn.functional.softmax(attention, dim=3) # 对列求softmax -> (N, H, T, T)
    output = torch.matmul(nn.functional.dropout(attention, dropout), value).transpose(1, 2).reshape(N, S, E)
    ############################################################################
    return output, attention
