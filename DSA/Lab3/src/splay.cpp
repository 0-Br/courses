    ...
    void splay(binode<T> *v)
    {
        ...
        while (p && g)
        {
            ...
            if (v -> is_lc())
            {
                if (p -> is_lc())//zig-zig
                {
                    BST<T>::attach_l(p -> rc, g);
                    BST<T>::attach_l(v -> rc, p);
                    BST<T>::attach_r(v, p);
                    BST<T>::attach_r(p, g);
                }
                else//zig-zag
            ...
            else
            {
                if (p -> is_rc())//zag-zag
                {
                    BST<T>::attach_r(g, p -> lc);
                    BST<T>::attach_r(p, v -> lc);
                    BST<T>::attach_l(p, v);
                    BST<T>::attach_l(g, p);
                }
                else//zag-zig
...
public:
    ...
    {
        if (Bitree<T>::empty()) return false;
        if (e != search(e) -> data) return false;
        binode<T>* L = Bitree<T>::_root -> binode<T>::lc;
        binode<T>* R = Bitree<T>::_root -> binode<T>::rc;
        release(Bitree<T>::_root);
        if (!R)
        {
            if (L) L -> parent = NULL;
            Bitree<T>::_root = L;
...
int main()
{
    Splaytree<int>* ptree = new Splaytree<int>();
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        int k;
        scanf("\n%c %d", &type, &k);
        if (type == 'A') ptree -> insert(k);
        if (type == 'B') ptree -> remove(k);
        if (type == 'C')
        {
            binode<int> *x = ptree -> find(k);
            if (x) printf("%d\n", x -> data);
            else printf("-1\n");
        }
        //ptree -> trav_in(print);
