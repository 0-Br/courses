...
protected:
    ...
    //空树高度保护
    int stature(binode<T> *x){return ((x) ? x -> height : -1);}
    //空树规模保护
...
protected:
    ...
    //为节点连接左子树
    void attach_l(binode<T> *lc, binode<T> *p){p -> lc = lc;if (lc) lc -> parent = p;}
    //为节点连接右子树
    void attach_r(binode<T> *p, binode<T> *rc){p -> rc = rc;if (rc) rc -> parent = p;}
...
public:
    ...
    {
        //特判退化情况
        if (Bitree<T>::empty() || e == Bitree<T>::_root -> data)
        {
            _hot = NULL;
            return Bitree<T>::_root;
        }
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int type, op;
        scanf("%d %d", &type, &op);
        if (type == 1) ptree -> insert(op);
        if (type == 2) ptree -> remove(op);
        if (type == 3) printf("%d\n", (ptree -> find(ptree -> size() - op)) -> data);
